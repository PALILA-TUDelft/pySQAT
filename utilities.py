...
def get_bark(N: int, qb, freqs):
    """
    Convert a list/array of frequencies (Hz) into Bark values at
    selected FFT-bin positions.

    ORIGINAL MATLAB CODE:
    Get_Bark.m [Last Update: 12/05/2023]

    PYTHON IMPLEMENTATION:
    Gerard Mendoza Ferrandis - 07/05/2025

    Parameters:
        N (int): FFT length (used only to size the output array)
        qb (1-D index-like (list, tuple, ndarray)): Positions in the returned Bark vector
            that must be filled. Typical use: FFT-bin indices for the positive-frequency half.
        freqs (1-D array-like): Frequencies in Hz corresponding to `qb` (same length/shape).

    Returns:
        Bark (np.ndarray): Length ≈ N/2 + 1.  Zeros everywhere except at `qb`, which hold
            the Bark numbers interpolated from Zwicker’s table.
        Bark_raw (np.ndarray): The original Bark table: [band#, f_low, f_high, mid-Bark].
    """
