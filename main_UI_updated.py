...
class SQATApp(tk.Tk):
    ...
    def _refresh_params(self):
        ...
        if metric in ("Loudness (ISO 532-1)", "Loudness (ECMA 418-2)", "Sharpness (DIN 45692)", "Tonality (Aures 1985)",
                      "Tonality (ECMA 418-2)", "Annoyance (Di 2016)", "Annoyance (Zwicker 1999)", "Annoyance (More 2010)"):
        ...
        # ECMA loudness also supports method selection
    ...
    def on_analyze(self):
        ...
        # ECMA loudness accepts a method selection as well
        if metric == "Loudness (ECMA 418-2)":
            params["method"] = int(self.var_method.get())
        if metric == "Sharpness (DIN 45692)":
    ...
    def _run_metric(self, metric: str, p: Dict[str, Any], show_plots: bool):
        ...
        # ECMA (Sottek hearing model) wrappers
        if metric == "Loudness (ECMA 418-2)":
            try:
                from metrics_ecma import Loudness_ECMA418_2
            except Exception as e:
                raise RuntimeError(f"ECMA loudness unavailable: {e}")
            out = Loudness_ECMA418_2(insig=insig, fs=fs, field=p.get("field", 0), method=p.get("method", 1), time_skip=p.get("time_skip", 0), show=show_plots)
            return out
        if metric == "Roughness (ECMA 418-2)":
            ...
            out = Roughness_ECMA418_2(insig=insig, fs=fs, field=p.get("field", 0), method=p.get("method", 1), time_skip=p.get("time_skip", 0), show=show_plots)
            return out
        if metric == "Tonality (ECMA 418-2)":
            ...
            out = Tonality_ECMA418_2(insig=insig, fs=fs, field=p.get("field", 0), method=p.get("method", 1), time_skip=p.get("time_skip", 0), show=show_plots)
            # normalize tonality keys for the GUI
    ...
    def _summarize(self, metric: str, out: Dict[str, Any]) -> str:
        ...
        family = {
            ...
            "Sharpness (DIN 45692)":"S","Roughness (Daniel 1997)":"R","Roughness (ECMA 418-2)":"R",
            "Fluctuation Strength (Osses 2016)":"FS","Tonality (Aures 1985)":"K","Tonality (ECMA 418-2)":"T",
    ...
    def open_graph_window(self):
        ...
        elif metric == "Loudness (ECMA 418-2)" and "InstantaneousLoudness" in out: y_key = "InstantaneousLoudness"
        ...
        elif metric == "Roughness (ECMA 418-2)" and "InstantaneousRoughness" in out: y_key = "InstantaneousRoughness"
        ...
        elif metric == "Tonality (ECMA 418-2)" and "InstantaneousTonality" in out: y_key = "InstantaneousTonality"
