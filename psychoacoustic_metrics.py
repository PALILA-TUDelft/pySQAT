...
def Roughness_Daniel1997(insig=None, fs=None, time_skip=None, show=None):
    ...
    if not (fs == 44100 or fs == 40960 or fs == 48000):
        gcd_fs = math.gcd(48000, int(fs))  # greatest common denominator
        # resample requires integer ratios for up/down sampling
        # The length of the output signal is len(audio) * (target_fs / original_fs)
        audio = resample(audio, int(len(audio) * (48000 / fs)))
        fs = 48000
    ...
    # MinBf needs to be indexed by absolute Barkno values, not indices
    # MinExcdB is already indexed by absolute frequency bins (k_barkno_indices)
    # MinBf should correspond to the hearing threshold at specific Bark band centers/borders
    # The original MATLAB code uses MinBf = MinExcdB(zb);
    # This means MinBf is the hearing threshold at the frequencies corresponding to zb.
    # Since MinExcdB is indexed by k_barkno_indices (absolute freq bins),
    # zb should be absolute freq bins.
    # However, Cf and Bf are defined relative to N0.
    # Let's assume MinBf is indexed by the bark band number (0-46)
    # The original MATLAB code has a potential inconsistency here.
    # For now, let's keep the direct translation of MinBf = MinExcdB[zb]
    # but note that it's indexing MinExcdB (which is freq-indexed) with bark-band-related indices.
